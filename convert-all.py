...
class Converter:
    ...
    def run(self):
        ...
        result['camp_site_photos'] = self._do_photos()
        with open('leaflet-website/data.js', 'w') as fh:
            fh.write('// generated by convert-all.py\n')
            for k in sorted(result.keys()):
                fh.write(f'var {k}_tracks = {json.dumps(result[k])};\n')
            fh.write(f'var camp_site_photos = {result["camp_site_photos"]};\n')
            fh.write(f'var max_cpm = {max_cpm};\n')

    def _do_photos(self):
        result = []
        logger.info('Checking EXIF on photos')
        for f in sorted(
            ...
            if 'gps_latitude' not in dir(i):
                ...
                continue
            lat = dms_to_dec(i.gps_latitude)
            if i.gps_latitude_ref == 'S':
                lat = -1.0 * lat
            lon = dms_to_dec(i.gps_longitude)
            if i.gps_longitude_ref == 'W':
                lon = -1.0 * lon
            alt = 0
            if hasattr(i, 'gps_altitude'):
                alt = i.gps_altitude
            result.append({
                'lat': lat, 'lng': lon, 'alt': alt,
                'meta': {'name': name}
            })
        logger.info('Done with exif for %d photos', len(result))
        return result
